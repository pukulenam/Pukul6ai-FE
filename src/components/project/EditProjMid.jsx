...
function EditProjMid({users, project}) {
    ...
    const [details, setDetails] = useState(
        {
            ...
            start: datetimeToStr(project?.start),
            end: datetimeToStr(project?.end),
        }
    );
    ...
    function submitHandler() {
        ...
        axios.put('http://localhost:8000/api/project', details, {headers: {"Authorization" : `Bearer ${sessionStorage.getItem('token')}`}})
        .then(res => {
            navigate('/project');
            setLoading(false);
        })
        ...
        <div className="grid grid-col-1 lg:grid-col-12">
            <div className="col-span-1 lg:col-span-12 m-4 bg-orange-400 border border-black rounded text-left p-4 text-white flex flex-col">
                ...
                <div className="relative">
                    <input type='text' className={`w-2/3 rounded h-8 text-black px-2`} autoComplete="off" name="user" id="user" onBlur={() => setShow(!show)} onFocus={() => setShow(!show)} onChange={filter} value={details.userid} />
                ...
                <div className="relative">
                    <input type='text' autoComplete="off" className={`w-2/3 rounded h-8 text-black px-2`} name="user" id="user" onBlur={() => setShow2(!show2)} onFocus={() => setShow2(!show2)} onChange={filter2} value={details.adminid} />
                ...
                <div className="flex items-center mx-auto">
                    <button className="my-4 mx-2 rounded-lg p-3 bg-black border border-white" onClick={submitHandler}>Edit Project</button>
                    <Link to={`/project`}>
