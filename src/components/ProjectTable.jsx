...
function ProjectTable({project, user_role}) {
    ...
    function deleteHandler(id, name) {
        ...
        if (ok) {
            setLoading(true);
            axios.delete("http://localhost:8000/api/project", {headers: {"Authorization" : `Bearer ${sessionStorage.getItem('token')}`}, data: {id: id}})
            .then(res => {
                setLoading(false);
                setConf(res.data);
                setShow(true);
                        ...
                        <tr key={id}>
                            ...
                            {user_role === "admin" ? (
                                <td className='bg-orange-300 text-center w-24'>
                                    <button className='bg-yellow-400 p-1 lg:px-6 rounded border border-black shadow-lg'>edit</button>
                                </td>
                            ) : null
                            }
                            <td className='bg-orange-300 text-center w-24'>
                                <button className='bg-green-400 p-1 lg:px-6 rounded border border-black shadow-lg'>view</button>
                            </td>
                            {user_role === "admin" ? (
                                <td className='bg-orange-300 text-center w-20'>
                                    <button className='bg-red-100 rounded py-2 px-1 border border-black' onClick={() => deleteHandler(item.id, item.name)}>
